...
test('test_tooling_design_tokens_reverse_load', async t => {
  ...
  let version = await supernova.designSystemVersion(process.env.TEST_DB_DESIGN_SYSTEM_ID, process.env.TEST_DB_DESIGN_SYSTEM_VERSION_ID)
  ...
  let phase1TokenPack = phase1Tool.loadTokensFromObject(originalTokens)
