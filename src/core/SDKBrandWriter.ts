...
import { AnyToken, TokenType } from '..'
...
export class BrandWriter {
  ...
  async writeTokens(
    ...
    await this.dataCore.writeTokenData(
      this.brand.designSystemVersion.designSystem.id,
      this.brand.designSystemVersion,
      remoteTokens,
      remoteGroups,
      deleteTokens
    )
  ...
  async writeTheme(theme: TokenTheme): Promise<TokenThemeWriteResponse> {
    ...
    await this.dataCore.writeTokenThemeData(
      this.brand.designSystemVersion.designSystem.id,
      this.brand.designSystemVersion,
      remoteTheme
    )
