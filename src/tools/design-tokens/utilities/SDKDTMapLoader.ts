  ...
  weakValidateMapping(mapping: DTPluginToSupernovaMappingFile) {
    ...
    for (let map of mapPack) {
      ...
      if ((!map.tokenSets && !map.tokensTheme)) {
        throw SupernovaError.fromProcessingError('Unable to load mapping file: `mapping` must contain either `tokensTheme` or `tokenSets`')
      }
      if ((map.tokenSets && map.tokensTheme)) {
        throw SupernovaError.fromProcessingError('Unable to load mapping file: `mapping` must not contain both `tokensTheme` or `tokenSets`')
      }
      if (map.tokenSets && (!(map.tokenSets instanceof Array) || (map.tokenSets as Array<any>).length === 0)) {
        throw SupernovaError.fromProcessingError('Unable to load mapping file: `mapping`.`tokenSets` must be an Array with at least one entry')
      }
      if (map.tokensTheme && ((typeof map.tokensTheme !== "string") || (map.tokensTheme as string).length === 0)) {
        throw SupernovaError.fromProcessingError('Unable to load mapping file: `mapping`.`tokensTheme` must be a non-empty string')
      }
      if (!map.supernovaBrand || (typeof map.supernovaBrand !== "string") || (map.supernovaBrand.length === 0)) {
        throw SupernovaError.fromProcessingError('Unable to load mapping file: `supernovaBrand` must be a non-empty string')
      }
      if (map.supernovaTheme && ((typeof map.supernovaTheme !== "string") || (map.supernovaTheme.length === 0))) {
        throw SupernovaError.fromProcessingError('Unable to load mapping file: `supernovaTheme` may be empty but must be non-empty string if not')
      }
    }
  }

  processFileToMapping(mapping: DTPluginToSupernovaMappingFile): DTPluginToSupernovaMapPack {
